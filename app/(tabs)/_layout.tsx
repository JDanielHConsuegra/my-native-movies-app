...
const TabIcon = ({ icon, title, focused }: TabIconProps) => {
  ...
  return (
    <View>
      {focused ? (
        <View className="flex bg-accent flex-row w-full flex-1 min-w-[112px] min-h-16 mt-5 justify-center items-center rounded-full overflow-hidden">
          <Image source={icon} tintColor="#A8B5D8" className="size-5" />
          <Text className="text-gray-200 text-base font-semibold ml-2">
            {title}{" "}
          </Text>
        </View>
      ) : (
